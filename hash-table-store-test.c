...
{
    TCGv_hostptr hashedAddress = tcg_temp_local_new_hostptr_named("hashedAddress");
    gen_hash_address(env, hashedAddress, guestAddress);

    TCGv_i32 threadId = tcg_const_i32_named(get_thread_id(env), "threadId");
...
{
    TCGv_hostptr hashedAddress = tcg_temp_new_hostptr_named("hashedAddress");
    gen_hash_address(env, hashedAddress, guestAddress);
...
void gen_store_table_lock(CPUState *env, TCGv_i64 guestAddress)
{
    ...
    TCGv_hostptr lockAddress = tcg_temp_local_new_hostptr_named("lockAddress");
    tcg_gen_addi_i64(lockAddress, hashedAddress, offsetof(hst_entry_t, lock));

    TCGv_i32 expectedLock = tcg_const_local_i32_named(HST_UNLOCKED, "expectedLock");

    //  Acquiring the lock means storing this thread's id.
    TCGv_i32 newLock = tcg_const_local_i32_named(get_thread_id(env), "newLock");

    TCGv_i32 result = tcg_temp_local_new_i32_named("result");

    int retry = gen_new_label();
    gen_set_label(retry);

    //  Optimistically try to atomically acquire the lock (only succeeds if it's currently unlocked).
    tcg_gen_atomic_compare_and_swap_host_intrinsic_i32(result, expectedLock, lockAddress, newLock);
    ...
    tcg_gen_atomic_compare_and_swap_host_intrinsic_i32(result, newLock, lockAddress, newLock);
    ...
    tcg_gen_st_i64(guestAddress, cpu_env, offsetof(CPUState, locked_address));
...
void gen_store_table_unlock(CPUState *env, TCGv_guestptr guestAddress)
{
    ...
    tcg_gen_st_i64(null, cpu_env, offsetof(CPUState, locked_address));
