...
static void gen_push_pop(DisasContext *dc, uint8_t rlist, target_ulong spimm, bool is_push)
{
    ...
    if (is_push) {
...
#if defined(TARGET_RISCV64)
            tcg_gen_qemu_st64(reg, addr, dc->base.mem_idx);
#else
            tcg_gen_qemu_st32(reg, addr, dc->base.mem_idx);
#endif
        ...
        for (int i = s_regs - 1; i >= 0; i--) {
            int reg_num = (i <= 1) ? (8 + i) : (16 + i);
#if defined(TARGET_RISCV64)
            tcg_gen_qemu_ld64(reg, addr, dc->base.mem_idx);
#else
            tcg_gen_qemu_ld32s(reg, addr, dc->base.mem_idx);
#endif
    ...
    if (is_push) {
        gen_set_gpr(2, new_sp);
        gen_get_gpr(new_sp, 2);
    } else {
...
#if defined(TARGET_RISCV64)
    tcg_gen_movi_tl(table_addr, index << 3);
#else
    tcg_gen_movi_tl(table_addr, index << 2);
#endif
    tcg_gen_add_tl(table_addr, jvt, table_addr);
    
    // Load jump target from table (XLEN-bit load)
#if defined(TARGET_RISCV64)
    tcg_gen_qemu_ld64(target_addr, table_addr, dc->base.mem_idx);
#else
    tcg_gen_qemu_ld32u(target_addr, table_addr, dc->base.mem_idx);
#endif
    ...
    switch(funct3) {
        ...
        case 5: 
        {
            ...
            if (funct5 == 0x18 || funct5 == 0x1A || funct5 == 0x1E || funct5 == 0x1C) {
                if (!ensure_additional_extension(dc, RISCV_FEATURE_ZCMP)) {
                    break;
                }
                ...
                if ((funct2 == 0x1 && r1sc == r2sc)
                    || (riscv_has_ext(env, RISCV_FEATURE_RVE) && (r1sc > 1 || r2sc > 1))) {
                    kill_unknown(dc, RISCV_EXCP_ILLEGAL_INST);
                    break;
                }
                ...
                if (!ensure_additional_extension(dc, RISCV_FEATURE_ZCMT)) {
                    break;
                }
