...
void glue(helper_vnsrl_ivi, POSTFIX)(CPUState *env, uint32_t vd, int32_t vs2, target_long rs1)
{
    ...
    for (int ei = env->vstart; ei < env->vl; ++ei) {
        ...
        switch (eew) {
        case 8:
            ((uint8_t *)V(vd))[ei] = ((uint16_t *)V(vd))[vs2] >> rs1;
            break;
        case 16:
            ((uint16_t *)V(vd))[ei] = ((uint32_t *)V(vd))[vs2] >> rs1;
            break;
        case 32:
            ((uint32_t *)V(vd))[ei] = ((uint64_t *)V(vd))[vs2] >> rs1;
            break;
        default:
            helper_raise_exception(env, RISCV_EXCP_ILLEGAL_INST);
            break;
...
void glue(helper_vnsra_ivi, POSTFIX)(CPUState *env, uint32_t vd, int32_t vs2, target_long rs1)
{
    ...
    for (int ei = env->vstart; ei < env->vl; ++ei) {
        ...
        switch (eew) {
        case 8:
            ((int8_t *)V(vd))[ei] = ((int16_t *)V(vd))[vs2] >> rs1;
            break;
        case 16:
            ((int16_t *)V(vd))[ei] = ((int32_t *)V(vd))[vs2] >> rs1;
            break;
        case 32:
            ((int32_t *)V(vd))[ei] = ((int64_t *)V(vd))[vs2] >> rs1;
            break;
        default:
            helper_raise_exception(env, RISCV_EXCP_ILLEGAL_INST);
            break;
