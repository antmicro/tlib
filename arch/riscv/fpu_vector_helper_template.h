...
{                                                                                                                                       \
    const target_ulong eew = env->vsew;                                                                                                 \
    if (!does_vector_embedded_extension_support_float_for_eew(cpu, eew << 1)                                                            \
...
{                                                                                               \
    const target_ulong eew = env->vsew;                                                         \
    if (!does_vector_embedded_extension_support_float_for_eew(cpu, eew)                         \
        || V_IDX_INVALID(vs2) || env->vstart != 0) {                                            \
        raise_exception_and_sync_pc(env, RISCV_EXCP_ILLEGAL_INST);                              \
    }                                                                                           \
    float64 acc;                                                                                \
    switch (eew) {                                                                              \
...
{                                                                                                       \
    const target_ulong eew = env->vsew;                                                                 \
    if (!does_vector_embedded_extension_support_float_for_eew(cpu, eew)                                 \
        || V_IDX_INVALID(vd) || V_IDX_INVALID(vs2) || V_IDX_INVALID(vs1) || env->vstart != 0) {         \
        raise_exception_and_sync_pc(env, RISCV_EXCP_ILLEGAL_INST);                                      \
    }                                                                                                   \
    float64 acc;                                                                                        \
    switch (eew) {                                                                                      \
